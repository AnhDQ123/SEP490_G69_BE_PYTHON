...
U, sigma, Vt = svds(ratings_matrix.values.astype(float), k=1)
sigma = np.diag(sigma)
...
def get_collab_recommendations(user_id, top_n=10):
    ...
    return [
               {"productId": pid, "name": products_dict[pid], "description": descriptions_dict.get(pid, ""),
                "score": score}
               for pid, score in user_predictions.items() if pid in products_dict
           ][:top_n]
...
def get_content_recommendations(user_id, top_n=10, weight_purchased=0.3, weight_content=0.5):
    ...
    for item in items_to_predict:
        ...
        content_score = float(max(similarity_matrix[item_index]))

        bought_scores = []
        for p in purchased_items:
            if p in products_dict:
                similarity = float(similarity_matrix[item_index][list(products_dict.keys()).index(p)])
                quantity = user_purchases[user_purchases['productId'] == p]['quantity'].sum()  # Tính tổng số lượng mua
                bought_scores.append(similarity * quantity)

        bought_score = max(bought_scores) if bought_scores else 0.0  # Đảm bảo không bị lỗi
        total_score = content_score * weight_content + weight_purchased * bought_score
        total_score = 0.0 if np.isnan(total_score) else total_score  # Tránh NaN
    ...
    recommendations = sorted(
        [
            {
                "productId": int(pid),
                "name": str(products_dict[pid]),
                "description": str(descriptions_dict.get(pid, "")),
                "score": float(scaled_scores[i])
            }
...
def get_hybrid_recommendations(user_id, top_n=10, product_type="FRESH", alpha=0.5):
...
def get_popular_products(top_n, product_type):
    ...
    return [{"productId": pid}
            for pid in filtered_products["id"].sample(min(top_n, len(filtered_products)))]
